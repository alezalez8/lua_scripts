...
function request_data()
    -- send request for data and schedule read function 10ms later
    port:write(REQUEST_BYTE)
    return read_data, 10
end
...
function read_data()
    ...
    if #buffer == 0 then return request_data, run_interval_ms end
    if not valid_frame(buffer) then   -- discard frame
        ...
        return request_data, run_interval_ms
    end
